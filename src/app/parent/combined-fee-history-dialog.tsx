...
export function CombinedFeeHistoryDialog({ isOpen, setIsOpen, parent, feeSettings, selectedSession }: CombinedFeeHistoryDialogProps) {
    ...
    return (
        ...
        <Card className="print-area">
            <CardHeader className="p-4 bg-muted/30">
                ...
                <Badge variant="secondary" className="text-base font-bold tracking-wider">💰 COMBINED FEE SUMMARY ({selectedSession})</Badge>
                </div>
            </CardHeader>
            <CardContent className="p-4 space-y-6">
                ...
                <div>
                <h3 className="text-lg font-semibold mb-2">Children Summary for Session: {selectedSession}</h3>
                <div className="space-y-4">
                    {parent.children.map((child) => {
                    const { totalAnnualFee, totalPaid, due, previousSessionDue } = calculateAnnualDue(child, feeSettings, selectedSession);
                    if (child.session !== selectedSession && previousSessionDue === 0 && totalPaid === 0) {
                      return null; // Don't show children who are not in the selected session unless they have a balance history
                    }
                    return (
                ...
                <div>
                    <h3 className="text-lg font-semibold mb-2">Combined Payment History (All Time)</h3>
                    <Table>
                    <TableHeader>
                        <TableRow>
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4 text-center border-t pt-4">
                    ...
                    <p className="text-sm text-muted-foreground">Family Total Fees ({selectedSession})</p>
                    ...
                    <p className="text-sm text-muted-foreground">Family Previous Dues</p>
                     <p className="text-xl font-bold">Rs{parent.children.reduce((acc, c) => acc + calculateAnnualDue(c, feeSettings, selectedSession).previousSessionDue, 0).toFixed(2)}</p>
                    </div>
                    <div className="p-2 rounded-md bg-muted">
                    <p className="text-sm text-muted-foreground">Family Total Paid ({selectedSession})</p>
                    <p className="text-xl font-bold text-green-600">Rs{parent.totalPaid.toFixed(2)}</p>
                    </div>
                    <div className="p-2 rounded-md bg-muted">
                    <p className="text-sm text-muted-foreground">Family Balance Due ({selectedSession})</p>
