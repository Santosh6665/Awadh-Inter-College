...
import { getTeacherSalaryDataForMonth } from './actions';
import {
  Select,
  SelectContent,
  SelectItem,
...
export function SalaryView({ teacher }: SalaryViewProps) {
  ...
  const [salaryData, setSalaryData] = useState<{ attendance: any, holidays: string[] }>({ attendance: {}, holidays: [] });
  ...
  useEffect(() => {
    startTransition(async () => {
      try {
        const data = await getTeacherSalaryDataForMonth(currentMonth);
        setSalaryData(data);
      } catch (error) {
  ...
  const handleViewSlip = () => {
    const salaryDetails = calculateSalary(teacher, currentMonth, salaryData.attendance, salaryData.holidays);
  ...
  const salaryDetailsForMonth = useMemo(() => {
    // Ensure that salaryData.attendance is an object before passing it to calculateSalary
    const attendanceData = salaryData.attendance || {};
    return calculateSalary(teacher, currentMonth, attendanceData, salaryData.holidays);
  }, [teacher, currentMonth, salaryData]);
